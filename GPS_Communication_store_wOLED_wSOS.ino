...
struct SOSAlert {
  String espName;
  long timestamp;  // Store the timestamp when SOS was received
};

SOSAlert sosAlerts[MAX_SOS_ALERTS];
int sosAlertCount = 0;
bool sosActive;  // if an SOS is active 
SOSAlert currentSOS;
...
void OnDataRecv(const esp_now_recv_info_t *info, const uint8_t *incomingData, int len) {
  ...
  // Check if the incoming data is an SOS message
  if (receivedData.isSOS) {
    ...
    sosActive = true;  // Set flag that a new SOS has been received
  }
...
void sendSOS() {
    // Set the SOS flag to true
    TransmittedData dataToSend;
    strncpy(dataToSend.name, deviceName.c_str(), sizeof(dataToSend.name));
    dataToSend.gpsData = gpsDataToSend; // Include current GPS data
    dataToSend.isSOS = true;  // Set the SOS flag to true

    // Send the SOS data using ESP-NOW
...
void loop() {
    ...
    if (gpsDataToSend.latitude != 0 && gpsDataToSend.longitude != 0) {
      ...
      if (digitalRead(SOS_BUTTON_PIN) == LOW) {
        // Button is pressed, set the SOS flag
        sendSOS();
        delay(500); // debounce for button
    }

    //delay(1000);
    printESPData();
    if (sosActive) {
      if (millis() - sosStartTime < 5000) { //5 second
        displaySOSAlert(currentSOS);  // Show SOS
      }else {
        sosActive = false;  // Reset after duration
...
void outputESPData(float lat, float lon, long timestamp) {
  ...
  if (currentMillis - lastPageSwitch >= pageSwitchInterval) {
      currentPage = (currentPage + 1) % 3; // Toggle between 0 and 1
