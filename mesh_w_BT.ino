...
void sendBT(){
  for(int i=0; i< espNodeCount;i++){
    TransmittedData data_to_send;
    String msg;
    strcpy(data_to_send.name, espNodes[i].name);
    data_to_send.gpsData = espNodes[i].gpsData;
    msg = createJsonMessage(data_to_send);
    SerialBT.print(msg);
  }
}
...
bool connectBT() {
  bool btConnection = false;
  BTScanResults* devices = SerialBT.discover(10); // Scan for 10 seconds

  if (devices) {
    for (int i = 0; i < devices->getCount(); i++) { // Use getCount()
      BTAdvertisedDevice* device = devices->getDevice(i); // Get the device object

      if (device->haveName() && device->getName() == "ESP Reciever") { // Check device name
        if (SerialBT.connect(device->getAddress())) { // Connect using device address
          btConnection = true;
          break; // Stop scanning after successful connection
        }
      }
    }
  }
  return btConnection; // Return true if connection was successful
}
...
void loop() {
    ...
    if (digitalRead(BLUETOOTH_BUTTON_PIN) == LOW) {
      //display bluetooth attempted
        if(connectBT){
          sendBT();
        }
        delay(500);
